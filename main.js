...
function readBook() {
  readButtons.forEach(item => {
    item.onclick = function() {
      for (let i = 0; i < myLibrary.length; i++) {
        if (myLibrary[i].data === parseInt(this.parentNode.parentNode.getAttribute('data'))) {
          this.innerHTML === 'Unread' ? myLibrary[i].read = false : myLibrary[i].read = true
        }
      }
      if (this.innerHTML === 'Unread') {
        this.innerHTML = 'Read'
        this.parentNode.parentNode.querySelector('.book_read').innerHTML = 'Read: No'
      } else {
        this.innerHTML = 'Unread'
        this.parentNode.parentNode.querySelector('.book_read').innerHTML = 'Read: Yes'
      }
      saveLocally()
    }
  })
